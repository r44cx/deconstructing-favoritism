...
class JudgeOutputCollector:
    def __init__(self, judge_model, ollama_host="http://localhost:11434", 
                 temperature=0.0, top_p=None, top_k=None, seed=None, mode='pairwise'):
    ...
    def get_judge_output(self, judge_prompt, retries=3, num_predict=20):
        for attempt in range(retries):
            try:
                ...
                return {
                    'judge_output': response['response'].strip(),
    ...
    def evaluate_pairwise(self, eval_item, flip_responses=False):
        ...
        # Allow enough tokens for JSON response with reasoning
        judge_result = self.get_judge_output(judge_prompt, num_predict=1024)
    ...
    def collect_outputs(self, evaluation_set_file, output_file, save_interval=10):
        ...
        for i, eval_item in enumerate(evaluations):
            ...
            if completed_tasks % save_interval == 0 or len(conv_results) > 0:
...
def main():
    parser = argparse.ArgumentParser(description="Collect LLM judge outputs")
    ...
    collector = JudgeOutputCollector(
